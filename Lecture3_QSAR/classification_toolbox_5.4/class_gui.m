...
h1 = 'classification toolbox for MATLAB version 5.4';
hr = sprintf('\n');
...
[cv_groups,cv_type,~,scaling,assign_method,~,~,~,domodel] = visualize_settings_big('plsda',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,NaN,1);
...
if length(handles.data.class_labels) == 0
    ...
    class_labels = handles.data.class_labels;
end
[cv_groups,cv_type,num_comp,scaling,target_class,perc,~,~,domodel] = visualize_settings_big('pf',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,class_labels,1);
if ischar(num_comp)
    % exceptions
...
if domodel
    if length(handles.data.class_labels) == 0
        class_check = calc_class_modellinglabels(handles.data.class,target_class);
    else
        class_check = calc_class_modellinglabels(handles.data.class,target_class,handles.data.class_labels);
    end
    err_cv = check_cv_err(class_check,cv_type,cv_groups);
    if err_cv == 1
        if strcmp(cv_type,'vene')
            meth = 'venetian blinds';
        else
        ...
        % run model
        res = potsmootsel(handles.data.X,class_to_fit,target_class,perc,scaling,cv_type,cv_groups,num_comp);
        if numel(res.er) > 1
...
maxcomp = min([size(handles.data.X,2) size(handles.data.X,1)]);
if maxcomp > 10; maxcomp = 10; end
[cv_groups,cv_type,num_comp,scaling,kernel,~,~,~,domodel] = visualize_settings_big('svm',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,NaN,1);
if ischar(num_comp)
    % exceptions
    if strcmp(num_comp,'automatic')
        num_comp = 0;
...
[cv_groups,cv_type,~,scaling,target_class,~,~,~,domodel] = visualize_settings_big('simca',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,class_labels,1);
if strcmp('none',cv_type) && domodel == 1
    domodel = 0;
    errortype = (['the optimal number of components should be choosen in cross validation. Choose a cross validation type.']);
    errordlg(errortype,'calculation error')
...
if domodel
    ...
    if err_cv == 1
        ...
        res = simcacompsel(handles.data.X,class_to_fit,target_class,scaling,cv_type,cv_groups);
        if length(res.er) > 1
...
[cv_groups,cv_type,~,scaling,target_class,~,~,~,domodel] = visualize_settings_big('uneq',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,class_labels,1);
if strcmp('none',cv_type) && domodel == 1
    domodel = 0;
    errortype = (['the optimal number of components should be choosen in cross validation. Choose a cross validation type.']);
    errordlg(errortype,'calculation error')
...
if domodel
    ...
    if err_cv == 1
        ...
        res = uneqcompsel(handles.data.X,class_to_fit,target_class,scaling,cv_type,cv_groups);
        if length(res.er) > 1
...
maxcomp = min([size(handles.data.X,2) size(handles.data.X,1)]);
if maxcomp > 20; maxcomp = 20; end
[cv_groups,cv_type,~,scaling,method,~,~,~,domodel] = visualize_settings_big('pcada',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,NaN,1);
if strcmp('none',cv_type) && domodel == 1
    domodel = 0;
    errortype = (['the optimal number of components should be choosen in cross validation. Choose a cross validation type.']);
    errordlg(errortype,'calculation error')
...
% open do settings
[cv_groups,cv_type,~,scaling,dist_type,~,~,~,domodel] = visualize_settings_big('knn',size(handles.data.X,1),handles.present.model,handles.data.model,10,NaN,1);
if strcmp('none',cv_type) && domodel == 1
    domodel = 0;
...
maxcomp = min([size(handles.data.X,2) size(handles.data.X,1)]);
if maxcomp > 20; maxcomp = 20; end
[cv_groups,cv_type,num_comp,scaling,assign_method,~,~,~,domodel] = visualize_settings_big('plsda',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,NaN,0);
if domodel
    % activate pointer
    set(handles.class_gui,'Pointer','watch')
    % run model
...
[cv_groups,cv_type,num_comp,scaling,target_class,~,~,~,domodel] = visualize_settings_big('simca',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,class_labels,0);
if domodel
    % activate pointer
    set(handles.class_gui,'Pointer','watch')
    % run model
    ...
    model = simcafit(handles.data.X,class_to_fit,target_class,num_comp,scaling);
    if strcmp('none',cv_type)
        cv = [];
    else
        if length(handles.data.class_labels) == 0
        ...
        if err_cv == 1
            ...
            cv = simcacv(handles.data.X,class_to_fit,target_class,num_comp,scaling,cv_type,cv_groups);
        end
    end
    % check if model and cv are calculated
    if isstruct(model)
...
[cv_groups,cv_type,num_comp,scaling,target_class,~,~,~,domodel] = visualize_settings_big('uneq',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,class_labels,0);
if domodel
    % activate pointer
    set(handles.class_gui,'Pointer','watch')
    % run model
    ...
    model = uneqfit(handles.data.X,class_to_fit,target_class,num_comp,scaling);
    if strcmp('none',cv_type)
        cv = [];
    else
        if length(handles.data.class_labels) == 0
        ...
        if err_cv == 1
            ...
            cv = uneqcv(handles.data.X,class_to_fit,target_class,num_comp,scaling,cv_type,cv_groups);
        end
    end
    % check if model and cv are calculated
    if isstruct(model)
...
[cv_groups,cv_type,num_comp,scaling,target_class,perc,smoot,~,domodel] = visualize_settings_big('pf',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,class_labels,0);
if ischar(num_comp)
    % exceptions
    if strcmp(num_comp,'automatic')
        num_comp = 0;
...
if domodel
    ...
    model = potfit(handles.data.X,class_to_fit,target_class,smoot,perc,scaling,num_comp);
    if strcmp('none',cv_type)
        cv = [];
    else
         if length(handles.data.class_labels) == 0
        ...
        if err_cv == 1
            ...
            cv = potcv(handles.data.X,class_to_fit,target_class,smoot,perc,scaling,cv_type,cv_groups,num_comp);
        end
    end
    % check if model and cv are calculated
    if isstruct(model)
...
maxcomp = min([size(handles.data.X,2) size(handles.data.X,1)]);
if maxcomp > 10; maxcomp = 10; end
[cv_groups,cv_type,num_comp,scaling,kernel,param,cost,~,domodel] = visualize_settings_big('svm',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,NaN,0);
if ischar(num_comp)
    % exceptions
    if strcmp(num_comp,'automatic')
        num_comp = 0;
...
maxcomp = min([size(handles.data.X,2) size(handles.data.X,1)]);
if maxcomp > 20; maxcomp = 20; end
[cv_groups,cv_type,num_comp,scaling,method,~,~,~,domodel] = visualize_settings_big('pcada',size(handles.data.X,1),handles.present.model,handles.data.model,maxcomp,NaN,0);
if domodel
    % activate pointer
    set(handles.class_gui,'Pointer','watch')
    % run model
...
% open do settings
[cv_groups,cv_type,num_k,scaling,dist_type,~,~,~,domodel] = visualize_settings_big('knn',size(handles.data.X,1),handles.present.model,handles.data.model,10,NaN,0);
if domodel
    % activate pointer
    set(handles.class_gui,'Pointer','watch')
    % run model
...
% open do settings
[cv_groups,cv_type,alpha,iter,num_hidden_layers,num_neurons,learning_rate,doplot,domodel] = visualize_settings_big('backprop',size(handles.data.X,1),handles.present.model,handles.data.model,5,NaN,0);
if domodel
    % activate pointer
    set(handles.class_gui,'Pointer','watch')
    % run model
...
elseif strcmp(handles.data.model.type,'simca') || strcmp(handles.data.model.type,'uneq')
    ...
    str{2} = ['model type: ' upper(handles.data.model.type)];
    nc = num2str(size(handles.data.model.T,2));
    ev = num2str(round(handles.data.model.modelpca.cum_var(end)*100));
    scal = handles.data.model.settings.pret_type;
    if isnumeric(handles.data.model.settings.target_class)
        set_this = num2str(handles.data.model.settings.target_class);
    else
        set_this = handles.data.model.settings.target_class;
    end
    str{3} = ['target class: ' set_this];
    if strcmp(scal,'none')
        set_this = 'none';
    elseif strcmp(scal,'cent')
        set_this = 'mean centering';
    ...
    str{5} = ['PCs: ' nc];
    str{6} = ['explained var. (%): ' ev];
    er = round(handles.data.model.class_param.er*100)/100;
    ner = round(handles.data.model.class_param.ner*100)/100;
    str{7} = ['error rate: ' num2str(er)];
    cnt = 8;
    if isstruct(handles.data.model.cv)
        ercv = round(handles.data.model.cv.class_param.er*100)/100;
        cvtype = handles.data.model.cv.settings.cv_type;
...
elseif strcmp(handles.data.model.type,'pf')
    ...
    if isnan(handles.data.model.settings.num_comp)
        ...
        set_this = num2str(size(handles.data.model.settings.Xclass,2));
...
% ------------------------------------------------------------------------
function disp_roc_simca(handles)
[~,col_default] = visualize_colors;
model = handles.data.model;
figure
hold on
...
cnt = cnt + 1;
subplot(1,2,cnt)
TPR = model.settings.sn';
FPR = 1 - model.settings.sp';
area(FPR,TPR,'FaceColor',col_default(1,:));
box on
xlabel('1 - specificity')
ylabel('sensitivity')
...
AUC = round(AUC*100)/100;
if length(model.labels.class_labels) == 0
    title(['ROC curve - class ' num2str(model.settings.target_class) ' (AUC: ' num2str(AUC) ')'])
else
    title(['ROC curve - ' model.settings.target_class ' (AUC: ' num2str(AUC) ')'])
end
% specificity and sensitivity
cnt = cnt + 1;
subplot(1,2,cnt)
hold on;
plot(model.settings.thr_val(2:end-1),model.settings.sn(2:end-1)','b')
plot(model.settings.thr_val(2:end-1),model.settings.sp(2:end-1)','r')
axis([min(model.settings.thr_val(2:end-1)) max(model.settings.thr_val(2:end-1)) 0 1])
xlabel('threshold')
ylabel('sn and sp ')
if length(model.labels.class_labels) == 0
    title(['sensitivity (blue) and specificity (red) - class ' num2str(model.settings.target_class)])
else
    title(['sensitivity (blue) and specificity (red) - ' model.settings.target_class])
end
hold off
box on
...
[~,col_default] = visualize_colors;
eigen = handles.data.model.modelpca.E;
exp_var = handles.data.model.modelpca.exp_var;
num_comp = length(exp_var);
if isnumeric(handles.data.model.settings.target_class)
    target_class = num2str(handles.data.model.settings.target_class);
else
    target_class = handles.data.model.settings.target_class;
end
figure
cnt = 0;
cnt = cnt + 1;
...
if length(class_labels) == 0
    lab_class = ['class ' num2str(res.settings.target_class)];
else
    g = find(strcmp(res.settings.target_class,class_labels));
    lab_class = class_labels{g};
end
cnt = cnt + 1;
subplot(2,1,cnt)
hold on
...
subplot(2,1,cnt)
hold on
plot(res.sensitivity,'o','MarkerEdgeColor','k','MarkerSize',6,'MarkerFaceColor','k')
plot(res.specificity,'o','MarkerEdgeColor','k','MarkerSize',6,'MarkerFaceColor','w')
plot(res.sensitivity,'k')
plot(res.specificity,'k')
legend('sensitivity','specificity')
ylabel('sensitivity and specificity in cv')
ylim = get(gca, 'YLim');
axis([0.6 (num_values + 0.4) ylim(1) ylim(2)])
set(gca,'xtick',[1:num_values]);
set(gca,'xticklabel',xvalues);
...
if strcmp(errortype,'none')
    ...
    if handles.present.class == 1
        if strcmp(handles.data.model.type,'simca') || strcmp(handles.data.model.type,'uneq') || strcmp(handles.data.model.type,'pf')
            handles.data.pred.class = handles.data.class;
            if length(handles.data.model.labels.class_labels) == 0
                [class] = calc_class_modellinglabels(handles.data.class,handles.data.model.settings.target_class);
            else
                [class] = calc_class_modellinglabels(handles.data.class,handles.data.model.settings.target_class,handles.data.model.labels.class_labels);
            end
            handles.data.pred.class_classmodelling = class;
            handles.data.pred.class_param = calc_class_param_classmodelling(handles.data.pred.class_pred,class);
        else
            handles.data.pred.class = handles.data.class;
