...
if strcmp(model.type,'simca') || strcmp(model.type,'uneq')
    ...
    for p = 1:size(model.T,2)
        k = k + 1;
        str_disp{k} = [target_class ' loadings on PC ' num2str(p)];
        store_for_plot{k}.val = model.modelpca.L(:,p);
        lab = [target_class ' - loadings on PC ' num2str(p) ' - EV = ' num2str(round(model.modelpca.exp_var(p)*10000)/100) '%'];
        store_for_plot{k}.lab = lab;
        store_for_plot{k}.pred = [];
    end
end
