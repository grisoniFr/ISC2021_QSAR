...
elseif length(there_x) > 0 && strcmp(handles.model.type,'uneq')
    tlim = handles.model.settings.thr;
    if ~isnan(tlim); this = find_max_axis(x_lim(2),tlim); x_lim = [x_lim(1) this]; end
elseif length(there_x) > 0
    tlim = handles.model.tlim;
    if ~isnan(tlim); this = find_max_axis(x_lim(2),tlim); x_lim = [x_lim(1) this]; end
end
...
elseif strncmp(Lab_Tx,'calculated response',19) && strcmp(handles.model.settings.assign_method,'bayes')
    ...
    line([thrc thrc],y_lim,'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds)
...
elseif length(strfind(Lab_Tx,'distance')) > 0
    if ~strcmp(handles.model.type,'svm')
        if ~isnan(handles.model.settings.thr(1))
            dlim = handles.model.settings.thr;
            line([dlim dlim],y_lim,'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds);
        end
    else
        line([-1 -1],y_lim,'Color','k','LineStyle',':','LineWidth',LineWidth_for_thresholds);
        line([0 0],y_lim,'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds);
        line([1 1],y_lim,'Color','k','LineStyle',':','LineWidth',LineWidth_for_thresholds);
    end
elseif strncmp(Lab_Tx,'output',6)
    classin = x - 1;
    thrc = handles.model.settings.thr(classin);
    line([thrc thrc],y_lim,'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds)   
end
...
elseif strncmp(Lab_Ty,'calculated response',19) && strcmp(handles.model.settings.assign_method,'bayes')
    classin = y - 1;
    thrc = handles.model.settings.thr(classin);
    line(x_lim,[thrc thrc],'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds)
...
elseif length(strfind(Lab_Ty,'distance')) > 0
    if ~strcmp(handles.model.type,'svm')
        if ~isnan(handles.model.settings.thr(1))
            dlim = handles.model.settings.thr;
            line(x_lim,[dlim dlim],'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds);
        end
    else
        line(x_lim,[-1 -1],'Color','k','LineStyle',':','LineWidth',LineWidth_for_thresholds);
        line(x_lim,[0 0],'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds);
        line(x_lim,[1 1],'Color','k','LineStyle',':','LineWidth',LineWidth_for_thresholds);
    end
elseif strncmp(Lab_Ty,'output',6)
    classin = y - 1;
    thrc = handles.model.settings.thr(classin);
    line(x_lim,[thrc thrc],'Color','r','LineStyle',':','LineWidth',LineWidth_for_thresholds)
end
...
if strcmp(model.type,'simca')
    if isnumeric(model.settings.target_class)
        target_class = ['class ' num2str(model.settings.target_class)];
    else
    ...
    for p = 1:size(model.T,2)
        k = k + 1;
        str_disp{k} = ['[' target_class '] scores on PC ' num2str(p)];
        store_for_plot{k}.val = model.T(:,p);
        lab = ([target_class ' - scores on PC ' num2str(p) ' - EV = ' num2str(round(model.modelpca.exp_var(p)*10000)/100) '%']);
        store_for_plot{k}.lab = lab;
        store_for_plot{k}.pred = [];
        if isstruct(pred)
            store_for_plot{k}.pred = pred.T(:,p);
    ...
    % Q residuals
    k = k + 1;
    str_disp{k} = ['[' target_class '] Q residuals'];
    store_for_plot{k}.val = model.Qres;
    store_for_plot{k}.lab = [target_class ' - Q residuals'];
    store_for_plot{k}.pred = [];
    if isstruct(pred)
        store_for_plot{k}.pred = pred.Qres;
    end
    % Hoteling
    k = k + 1;
    str_disp{k} = ['[' target_class '] Hotelling T^2'];
    store_for_plot{k}.val = model.Thot;
    store_for_plot{k}.lab = [target_class ' - Hotelling T^2'];
    store_for_plot{k}.pred = [];
    if isstruct(pred)
        store_for_plot{k}.pred = pred.Thot;
    end
    % Class distances
    k = k + 1;
    str_disp{k} = ['[' target_class '] distance'];
    store_for_plot{k}.val = model.distance;
    store_for_plot{k}.lab = [target_class ' - normalised distance'];
    store_for_plot{k}.pred = [];
    if isstruct(pred)
        store_for_plot{k}.pred = pred.distance;
    end
end
if strcmp(model.type,'uneq')
    ...
    % Hoteling
    k = k + 1;
    str_disp{k} = ['[' target_class '] normalised Hotelling T^2'];
    store_for_plot{k}.val = model.Thot_reduced;
    store_for_plot{k}.lab = [target_class ' - normalised Hotelling T^2'];
    store_for_plot{k}.pred = [];
    if isstruct(pred)
        store_for_plot{k}.pred = pred.Thot_reduced;
    end
end
...
if strcmp(model.type,'pf')
    ...
    % potential
    k = k + 1;
    str_disp{k} = ['potential - ' target_class];
    store_for_plot{k}.val = model.P;
    store_for_plot{k}.lab = ['potential - ' target_class];
    store_for_plot{k}.pred = [];
    if isstruct(pred)
        store_for_plot{k}.pred = pred.P;
    end
end
