function class_param = calc_class_param_classmodelling(class_calc,class)

% calc_class_param_classmodelling calculates classification measures for class modelling methods
%
% class_param = calc_class_param(class_calc,class);
%
% INPUT:            
% class_calc        calculated class vector [samples x 1]
% class             class vector [samples x 1], numerical vector. Class labels must be 1 (modelled class)or 0
%
% OUTPUT:
...
% mdoelled class
in_class = find(class == 1);
conf_mat(1,1) = length(find(class_calc(in_class) == 1));
conf_mat(1,2) = length(find(class_calc(in_class) == 0));
conf_mat(1,num_class + 1) = 0;
% not mdoelled class
in_class = find(class == 0);
conf_mat(2,1) = length(find(class_calc(in_class) == 1));
conf_mat(2,2) = length(find(class_calc(in_class) == 0));
conf_mat(2,num_class + 1) = 0;
