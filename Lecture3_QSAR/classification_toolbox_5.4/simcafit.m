function model = simcafit(X,class,target_class,num_comp,pret_type)

% fit SIMCA model for a given target class
%
% model = simcafit(X,class,target_class,num_comp,pret_type)
%
% INPUT:
...
% qlim              Q residuals confidence limit (95th percentile)
...
% settings.thr      when using 'class modeling' as assign_method, this field contains the distance threshold
%
% RELATED ROUTINES:
...
Qres(find(class == modelled_class),:) = modelpca.Qres';
Qres(find(class ~= modelled_class),:) = modelpca.Qres_pred';
...
model.Qres = Qres;
model.Qres_reduced = Qres./modelpca.settings.qlim;
model.Qcont = Qcont;
model.qlim = modelpca.settings.qlim;
% pca model
model.modelpca = modelpca;
% calculate normaliased distance
for i=1:size(X,1)
    Q = model.Qres_reduced;
    T = model.Thot_reduced;
    q = Q(i);
    t = T(i);
    model.distance(i,1) = (q^2 + t^2)^0.5;
end
resthr = simcafindthr(model.distance,class);
model.class_calc = simcafindclass(model.distance,resthr.class_thr);
