function cv = simcacv(X,class,target_class,comp,pret_type,cv_type,cv_groups)

% cross-validation for SIMCA for a given target class
%
% cv = simcacv(X,class,target_class,comp,pret_type,cv_type,cv_groups)
%
% INPUT:
...
if strcmp(cv_type,'boot')
    ...
    for i=1:cv_groups
        ...
        x_in  = x(whos_in,:);
        y_in  = y(whos_in,:);
        model = simcafit(x_in,y_in,modelled_class,comp,pret_type);
