...
if get(handles.pop_yaxis,'Value') == 1
    pret_type = 'none';
    axis_label = 'no scaling';
elseif get(handles.pop_yaxis,'Value') == 2
    pret_type = 'cent';
    axis_label = 'centered data';
elseif get(handles.pop_yaxis,'Value') == 3
    pret_type = 'auto';
    axis_label = 'autoscaled data';
else
    pret_type = 'rang';
    axis_label = 'range scaled data';
end
...
if get(handles.pop_xaxis,'Value') == 1 % samples
    ...
    plot_mean(data_raw,handles.var_labels,'raw data')
end
% plot scaled data
if external
...
cla;
if get(handles.pop_xaxis,'Value') == 1 % samples
    plot_samples(data_scaled,handles.var_labels,axis_label)
else % average
    plot_mean(data_scaled,handles.var_labels,axis_label)
end
xlabel('variables')

% -------------------------------------------------------------------------
function plot_mean(X,variable_labels,add_ylabel)
