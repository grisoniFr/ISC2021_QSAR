function cv = bnncv(X,y,alpha,dist_type,pret_type,cv_type,cv_groups)

% cross-validate local regression based on Binned Nearest Neighbours (BNN)
%
% cv = bnncv(X,y,alpha,dist_type,pret_type,cv_type,cv_groups)
%
% INPUT:
...
if strcmp(cv_type,'boot')
    ...
    for i=1:cv_groups
        ...
        x_in  = X(whos_in,:);
        y_in  = y(whos_in);
        pred = bnnpred(x_ext,x_in,y_in,alpha,dist_type,pret_type);
