function cv = plscv(X,y,num_lv,pret_type,cv_type,cv_groups)

% cross-validate Partial Least Squares regression (PLS)
%
% cv = plscv(X,y,comp,pret_type,cv_type,cv_groups)
%
% INPUT:
...
if strcmp(cv_type,'boot')
    ...
    for i=1:cv_groups
        ...
        x_in  = X(whos_in,:);
        y_in  = y(whos_in);
        model = plsfit(x_in,y_in,num_lv,pret_type,0);
        pred = plspred(x_ext,model);
