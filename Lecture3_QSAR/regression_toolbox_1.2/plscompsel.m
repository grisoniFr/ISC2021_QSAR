function res = plscompsel(X,y,pret_type,cv_type,cv_groups)

% selection of the optimal number of latent variables for PLS by means of cross-validation
%
% res = plscompsel(X,y,pret_type,cv_type,cv_groups)
%
% INPUT:
...
% R2                R2 in cross-validation as a function of latent variables [1 x latent variables]
% rmse              root mean squared error in cross validation as a function of latent variables [1 x latent variables]
% settings          settings
%
% RELATED ROUTINES:
...
for k = 1:r
    if ~ishandle(hwait)
        ...
        waitbar(k/r)
        out = plscv(X,y,k,pret_type,cv_type,cv_groups);
