...
if length(handles.vars_listed) == 0
    ...
    elseif strcmp(handles.load_type,'data')
        errortype = check_data(handles);
    end
...
% response is string 
if isstr(response)                                     
    errortype = 'input error: response values must be numbers (strings are not allowed)';
    return
end
% no. of samples for data and response
if size(response,1) ~= handles.num_samples
    chk = 0;
    errortype = 'input error: data and response must have the same number of rows (objects)';
    return
end
% missing values
if sum(isnan(response)) > 0
    chk = 0;
    errortype = 'input error: response must not have missing values';
    return
end
% not costant
if min(response) == max(response)
    chk = 0;
    errortype = 'input error: response must not be constant';
    return
end

% -------------------------------------------------------------------------
function errortype = check_data(handles)
errortype = 'none';
if handles.output.from_file == 1
    tmp_data = load(handles.output.path);
    in = get(handles.listbox_variables,'Value');
    X = getfield(tmp_data,handles.vars_listed(in).name);
else
    in = get(handles.listbox_variables,'Value');
    X = evalin('base',handles.vars_listed(in).name);
end
% missing values
if sum(sum(isnan(X))) > 0
    chk = 0;
    errortype = 'input error: data must not have missing values';
    return
end
...
if ~isfield(model,'yc') | ~isfield(model,'type') | ~isfield(model,'settings') | ~isfield(model,'cv')
