function cv = knncv(X,y,K,dist_type,pret_type,cv_type,cv_groups)

% cross-validate local regression based on K Nearest Neighbours (KNN)
%
% cv = knncv(X,y,K,dist_type,pret_type,cv_type,cv_groups)
%
% INPUT:
...
if strcmp(cv_type,'boot')
    ...
    for i=1:cv_groups
        ...
        x_in  = X(whos_in,:);
        y_in  = y(whos_in);
        pred = knnpred(x_ext,x_in,y_in,K,dist_type,pret_type);
